...
| 5 | **Step 2b** - Approvals allowlists | MCP Allowlist `playwright:*` (browser) **and** Command Allowlist (paste this comma-delimited line: `python3, python, rm, mkdir, cat, ls, cp, pip3, pip, bash, node, git clone, chmod, which, open`); Browser Protection OFF; Run Mode = Allowlist (with Sandbox). **Both** allowlists - browser AND shell |
...
5. **Command Allowlist** -> add these commands (copy-paste this whole comma-delimited line into the field - the skills' Python helper scripts, file ops, **and the setup steps** all run without prompting):
   ```
...
> **Run Mode can silently reset to Auto-review (the default) - but the allowlist still works in Auto-review.** An approval-card checkbox ("Enable Auto-review" / "Run in sandbox" / "Always allow" - often auto-checked), a Cursor restart, or a version update can flip Run Mode back to Auto-review. **This does NOT disable the allowlists** - Cursor checks the MCP and Command Allowlists first in every mode, so `playwright:*` and the allowlisted commands still run instantly with no card. The only difference is the fallback for *non-allowlisted* calls: Auto-review sends them to the LLM classifier (which can block enrichment-style profile extraction as "third-party data collection"), while Allowlist (with Sandbox) runs them sandboxed or asks. So with a complete allowlist the skills still run approval-free in Auto-review - but if a non-allowlisted call gets blocked by the classifier, re-set **Settings -> Agents -> Run Mode = Allowlist (with Sandbox)** and Cmd/Ctrl+Shift+P -> "Developer: Reload Window". To prevent the reset recurring, **uncheck the "Enable Auto-review" box on approval cards** before clicking Run/Allow. See `BROWSER_SETUP.md` troubleshooting.

...
- **Run Mode keeps resetting back to Auto-review / "regular sandbox" - but the allowlist still works.** This is a known Cursor behavior. An approval-card checkbox ("Enable Auto-review" / "Run in sandbox" / "Always allow" - often **auto-checked**, so clicking Run/Allow silently flips global Run Mode back to **Auto-review**, the default) is the usual culprit; a Cursor restart or version update can also reset it. **Important: this does NOT disable the allowlists.** Cursor checks the MCP and Command Allowlists first in *every* Run Mode, so `playwright:*` and the allowlisted commands still run instantly with no card in Auto-review too. The only difference is the fallback for *non-allowlisted* calls: Auto-review sends them to the LLM classifier (which can block enrichment-style profile extraction as "third-party data collection"), while Allowlist (with Sandbox) runs them sandboxed or asks. So with a complete allowlist the skills still run approval-free in Auto-review - you don't have to panic-fix it mid-run. If a non-allowlisted call does get blocked by the classifier, re-set **Settings -> Agents -> Run Mode = Allowlist (with Sandbox)**, then **Cmd/Ctrl+Shift+P -> "Developer: Reload Window"**. To prevent the reset recurring: **uncheck the "Enable Auto-review" box on approval cards** before clicking Run/Allow. (If you'd rather it never reset, the fully-automatic "Auto" Run Mode has no classifier and no approval cards to interact with - but it removes the guardrail for all agent actions, not just the skills.)

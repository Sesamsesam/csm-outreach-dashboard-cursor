...
- **Run Mode keeps resetting back to Auto-review / "regular sandbox".** This is a known Cursor behavior with two causes. (1) **An approval-card checkbox silently flips global Run Mode**: when an approval card appears mid-run, it often has a checkbox like "Enable Auto-review" / "Run in sandbox" / "Always allow" - checking it (and sometimes just interacting with the card) changes your **global** Run Mode back to **Auto-review**, which is the default. Since you click through approval cards during scrape/enrichment, this is the usual culprit. (2) A **Cursor restart or version update** can also reset Run Mode to the default. Fix: re-set **Settings -> Agents -> Run Mode = Allowlist (with Sandbox)**, then **Cmd/Ctrl+Shift+P -> "Developer: Reload Window"** to make it stick. To prevent it recurring: **do not check "Enable Auto-review" / "Run in sandbox" / "Always allow" on approval cards** - just click Run/Allow without toggling that box. After any session where you touched an approval card, re-verify Run Mode. (If you'd rather it never reset, the fully-automatic "Auto" Run Mode has no classifier and no approval cards to interact with - but it removes the guardrail for all agent actions, not just the skills.)
